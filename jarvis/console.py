...
def _char_input(prompt: str, kbhit, getwch, grace: float = 0.05,
    ...
    * Enter submits only when the input buffer is idle afterwards - the
      newlines inside a paste are kept, so a paste stays ONE prompt.
    """
    ...
    def update_menu() -> None:
        nonlocal menu_on
        cur = "".join(buf)
        col = (vis_prompt if not lines else 0) + len(buf) + 1
    ...
    while True:
        ...
        if ch in "\r\n":
            ...
            if pending is None and not kbhit():    # idle buffer: a real Enter
                if menu_on:
                    menu(None, 1)
                echo("\n")
        ...
        if ch == "\x16":                           # Ctrl+V: image clipboard
            ...
            update_menu()
            continue
        if ch in ("\x00", "\xe0"):                 # arrow/function key pair
            getwch()                               # consume and ignore
            continue
        buf.append(ch)
        echo(ch)
        update_menu()
...
def _status_bar(cfg: Config) -> str:
    ...
    return (f"  {_c('brain', 'grey')} {_c(cfg.brain.backend, 'cyan')}"
            ...
            f"{_c('voice', 'grey')} {dot(cfg.voice_enabled)}   "
