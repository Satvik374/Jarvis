...
def build_live_voice_system_prompt() -> str:
    ...
    return (
        ...
        "   - An on-screen control with no accessible name - an icon, a canvas, a picture, a game "
        "surface - is still reachable: 'click_target' reads the pixels with OCR when the element "
        "list cannot name what you asked for. So describe the words you can see on it instead of "
        "assuming nothing can be clicked.\n"
        "   - Every click re-reads the screen and tells you what actually changed. If the result says "
        "nothing has visibly changed, do NOT report success - check with 'look_at_screen' first. "
        "Pass 'retry': true only when the control is safe to activate twice; never for a checkbox, "
        "a toggle or a button that submits something.\n"
